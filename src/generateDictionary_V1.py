...
class HOCR:
    """
    Defines an HOCR entity from a pandas dataframe
    """
    def __init__(self, input_df, mean_shift_thresh=bandwidth_q):
        """
        Initializes the HOCR table.
        ...
        self._line2members = self._raw_hocr.groupby('line_num')['text'].apply(lambda g: g.values.tolist()).to_dict()
        ...
        for para in data:
            for line in data[para]:
                ...
                for line_hocr in linedict:
                    tot = self._raw_hocr[self._raw_hocr["line_num"] == line_hocr].shape[0]
                    match = 0
                    for tok in tokens:
    ...
    def get_wLeft(self, word, line):
        hocr = self._raw_hocr
        sample = hocr.loc[hocr['line_num']==line,:]
        #print("")
        #print(line)
        #print("")
        #print(sample)
    ...
    def returnDictionaryItems(self):
        ...
        def combineMultiColumn(first, second, deletion_index, firstcol, secondcol, stopcolumn, group_id):
            ...
            if deletion_index in self._line2members:
                n_first = data.loc[(data['line_num'] == deletion_index) & (data['column'] >= firstcol) & (data['column'] < secondcol), 'text'].str.cat(sep=' ')
                if stopcol != False:
                    n_second = data.loc[(data['line_num'] == deletion_index) & (data['column'] >= secondcol) & (data['column'] <= stopcol), 'text'].str.cat(sep=' ')
                else:
                    n_second = data.loc[(data['line_num'] == deletion_index) & (data['column'] >= secondcol), 'text'].str.cat(sep=' ')
                if len(first) == 0:
                    first = " "
                if len(second) == 0:
                    second = " "
            ...
            for range in rangedict:
                if linenum >= range[0] and linenum <= range[1]:
                    ...
                    for idx, group in enumerate(list(chunks(rangedict[range], 2))):
                        ...
                        try:
                            first = data.loc[(data['line_num'] == linenum) & (data['column'] >= firstcol) & (data['column'] < secondcol), 'text'].str.cat(sep=' ')#, self._raw_hocr['text']].str.join(' ')
                            if stopcol != False:
                                second = data.loc[(data['line_num'] == linenum) & (data['column'] >= secondcol) & (data['column'] <= stopcol - 2), 'text'].str.cat(sep=' ')
                            else:
                                second = data.loc[(data['line_num'] == linenum) & (data['column'] >= secondcol), 'text'].str.cat(sep=' ')
...
"""
class Column:
    def __init__(self, hocrobj, column_cluster_id):
        self._col_index = column_cluster_id
        self._members = [Word(member) for member in hocrobj._raw_hocr[hocrobj._c_lookup[column_cluster_id]].iterrows()]

class Line:
    def __init__(self, hocrobj, line_cluster_id):
        self._line_index = line_cluster_id
        self._members = [Word(member) for member in hocrobj._raw_hocr[hocrobj._l_lookup[line_cluster_id]].iterrows()]
...
with open('dictV3.csv', 'w', encoding='utf-8') as csv_file:
